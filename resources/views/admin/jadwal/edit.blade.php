...
<div class="content-wrapper">
    ...
    <div class="card">
        <div class="card-body">
            ...
            @endif
            <!-- form validasi -->
            <form class="forms-sample" action="{{ url('/jadwal/update', $jadwal->id)}}" method="POST">
                @csrf
                @method('PUT')
                <div class="form-group row">
                    <label for="id_lokasi" class="col-sm-4 col-form-label">Lokasi Id</label>
                    <div class="col-sm-8">
                    <select name="id_lokasi" id="id_lokasi" name="id_lokasi" class="form-control">
                        @foreach($lokasi as $lokasi)
                            <option value="{{ $lokasi->id }}"{{ ($lokasi->id == $jadwal->id_lokasi)? 'selected' : '' }}>{{ $lokasi->nama_lokasi}}
                            </option>
                        @endforeach
                    </select>
                    </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-8">
                        <input type="text" class="form-control" id="biaya" name="biaya"
                            placeholder="Biaya" value="{{ $jadwal->biaya}}">
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col-sm-8">
                        <input type="text" class="form-control" id="kuota" name="kuota"
                            placeholder="kuota" value="{{ $jadwal->kuota }}">
                    </div>
                </div>
